...
- **ManagementClass (MGMTCLAS)**: バックアップ・退避・期限切れ削除の運用ポリシ。HSM 連携（→ [ZOS-RECOVERY-001](zos-recovery-001.md)）。「捨てる残す軸」。
...
- データセットの DSORG / RECFM / SPACE の意味 — [ZOS-DATASET-001](zos-dataset-001.md)
- DASD ボリューム概念（VOL=SER） — [ZOS-DASD-001](zos-dasd-001.md)
- カタログとの関係（SMS-managed は必ずカタログ） — [ZOS-CATALOG-001](zos-catalog-001.md)
- 一般 IT 知識: ポリシ駆動ストレージ管理（Storage Class on Kubernetes 等と概念は近い）

## 4. relations（他アトムとの繋がり）

- `depends_on`: [ZOS-DATASET-001](zos-dataset-001.md), [ZOS-DASD-001](zos-dasd-001.md), [ZOS-CATALOG-001](zos-catalog-001.md)
- `specialized_by`: なし
- `contrasts_with`: 「非 SMS（直接 VOL=SER 指定）」運用との対比
- `used_by`: [ZOS-GDG-001](zos-gdg-001.md)（GDG base + SMS DataClass で世代生成）, [ZOS-RECOVERY-001](zos-recovery-001.md)（HSM が ManagementClass 経由で migrate）, [ZOS-VSAM-001](zos-vsam-001.md)（VSAM の CISIZE 等を DataClass で集中設定）
