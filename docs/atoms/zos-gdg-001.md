...
- データセット概念と DSN 命名規則 — [ZOS-DATASET-001](zos-dataset-001.md)
- カタログのエントリ種別（NONVSAM / GDG base / GDS） — [ZOS-CATALOG-001](zos-catalog-001.md)
- JCL の DISP パラメータ — [ZOS-JCL-001](zos-jcl-001.md)
...
- `used_by`: [ZOS-JCL-001](zos-jcl-001.md)（DD で相対世代指定）, [ZOS-SORT-001](zos-sort-001.md)（GDG 全世代 concatenation で集計）, [ZOS-RECOVERY-001](zos-recovery-001.md)（HSM が GDS 単位で migrate）
