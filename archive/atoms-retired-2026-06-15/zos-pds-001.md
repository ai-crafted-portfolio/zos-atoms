...
- DASD のシリンダ・トラック構造（→ ZOS-DASD-001）
- メンバ名規約（1〜8 文字、英数字 + `@#$`）
...
- `specialized_by`: なし
- `contrasts_with`: ZOS-USS-001（POSIX ディレクトリ）
- `used_by`: ZOS-JCL-001 (JCL ライブラリ), ZOS-CICS-001 (LOAD ライブラリ), ZOS-DB2-001 (DBRM ライブラリ)
...
市販書籍 (BK_MF_001, BK_ZOS_BASIC_001, BK_ZOS_TECH_002) から PDS / PDSE 管理の知識を概念蒸留 (ADR-0109)。逐語引用禁止。
