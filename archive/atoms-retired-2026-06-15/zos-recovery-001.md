...
Linux / クラウドなら `rsync + tape`, `restic`, `S3 + Glacier` 等の組合せで階層化するが、z/OS では **HSM が SMS の ManagementClass と統合されており、データセット属性に「いつ migrate するか・どこに行くか」を貼って自動運用** できる。バックアップは別個ツールではなく **ストレージ管理の延長**。これが SMS 必須の理由でもある（→ ZOS-SMS-001）。
...
- DASD ボリューム — `ZOS-DASD-001`
- SMS の ManagementClass — `ZOS-SMS-001`
- カタログ（migrate 後は catalog が ML1/ML2 を指す） — `ZOS-CATALOG-001`
- 一般 IT 知識: RPO / RTO の概念

## 4. relations（他アトムとの繋がり）

- `depends_on`: ZOS-DATASET-001, ZOS-DASD-001, ZOS-SMS-001
...
- `used_by`: ZOS-GDG-001（GDS の世代別 migrate）, ZOS-VSAM-001（VSAM の backup + RLS）, ZOS-DB2-001（Db2 image copy は別系統だが HSM tier に乗せる事例多い）
